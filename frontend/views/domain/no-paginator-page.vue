...
export default {
    ...
    computed: {
        ...
        idKey() {
            return this.defaultIdKey || (this.domain && this.domain.primaryKey) || 'id'
        },
        ...
        listActions() {
            return this.actions.map(it => {
                ...
                return defaultAction ? _.assign(_.cloneDeep(defaultAction), action) : action

            }).filter(it => !!it)
        },

        listPageProps() {
            return this.size === 'small' ? _.assign(_.cloneDeep(this.pageProps), { size: 'small' }) : this.pageProps
        }
    },
    ...
    mounted() {
        // 监听Domain事件
        if (this.domain) {
            App.event.$on(this.domain.events.Update, () => setTimeout(this.load, 300))
            App.event.$on(this.domain.events.Create, () => setTimeout(this.load, 300))
            App.event.$on(this.domain.events.Delete, () => setTimeout(this.load, 300))
        }
    },
