...
<template>
    <div>
        ...
        <form-item label="已选账号" v-if="selections.length>0">
...
import selectorT from '../selecter_people/selecter'
...
export default {
  ...
  methods: {
    ...
    toFromKey:function(){
        ...
        if(this.selections && this.selections.length){
            ...
            for(var i = 0;i < this.selections.length;i++){
                returnArray.push(this.selections[i].platformUid);
            } 
        }
    ...
    //传递给vuex
    ,handleToStore:function(){
        var returnArray = [];
        if(this.selections && this.selections.length){
            //这里要处理数据暂时不知道格式
            for(var i = 0;i < this.selections.length;i++){
                returnArray = returnArray.concat(this.selections[i].platformTags);
            } 
        }
        this.$store.state.idArgs = _.uniq(returnArray);
    }
    //ceshi
    ,testmodel:function(){
