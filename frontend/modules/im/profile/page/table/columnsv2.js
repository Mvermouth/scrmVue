...
const options = {
		...
		render(h, context) {
			return <div>
                <span class='info-value ellipsis' >
                    <tooltip transfer content={context.row.nickname}>{context.row.nickname}</tooltip>
                </span>
            </div>
		}
	},
	online: {
		...
		render(h, context) {
			return  <div>
				<span>运营状态:{context.row.online == 1 ? "在线" : "离线"}</span><br/>
				<span>设备UDID:{context.row.deviceID}</span>
