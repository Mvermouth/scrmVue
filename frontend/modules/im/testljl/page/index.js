...
import Testljl from '../../../../models/im/testljl'
...
const fillData = async(page) => {
	...
	// 查询重复好友数
	const dupProfiles = contactIds.length > 0 ? (await Testljl.batchRetrieveDuplicatedContactsCount(contactIds)) : { content: {}}
		...
		render: h => {
			const v = h(View, { props: {
				loadPage: async(pageNo, pageSize, filter, sort) => {
					const page = await Testljl.page(pageNo, pageSize, filter, sort)
					try {
						await fillData(page)
					} catch (error) {
